class ChatsController < ApplicationController
  ...
  def chat_room
    ...
    if User.find_by(id: params[:receiver])
      ...
      @messages = Message.where(["user_id = ? and receiver_id = ? OR user_id = ? and receiver_id = ?", current_user.id, params[:receiver].to_i, params[:receiver].to_i, current_user.id]).order(created_at: :asc).pluck(:body, :user_id, :created_at, :id)
